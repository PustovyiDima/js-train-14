...
console.log(setSpecificTime(new Date("2023-12-25T00:00:00.000Z"), 9, 30, 0, 0));
...
function nextNewYear() {
   ...
   let diff = Date.parse(newDate) - Date.parse(date);
   let obj = {
...
function isLeapYear(year) {
   ...
   if (typeof year === "number") {
      ...
      } else return { year: year, isLeap: false };
   }
...
function getDayOfWeek(date) {
   ...
   if (typeof date.getTime === "function") {
      ...
      let dayOfWeek = daysOfWeek[time - 1];
...
function getDaysInMonth(date) {
   ...
   if (typeof date.getTime === "function") {
      ...
      let nextDate = new Date(Date.UTC(year, ++month, 1));
      nextDate.setDate(nextDate.getDate() - 1);
      // console.log(nextDate.getDate());
      return {
         inputDate: inputDate.toISOString(),
         daysInMonth: nextDate.getDate(),
      };
   } else return "Помилка: вхідне значення має бути об'єктом Date";
}
...
function getFormattedTime(date) {
   ...
   if (typeof date.getTime === "function") {
      ...
      return { inputDate: date.toISOString(), formattedTime: res };
   } else return "Помилка: вхідне значення має бути об'єктом Date";
}
console.log("Завдання: 10 ==============================");

console.log(getFormattedTime(new Date("2023-01-25T09:30:00.000Z")));
console.log("Оскільки зимовий час +2 то відповідно до умови 09:30 стане 11:30");
console.log("А якщо місяць в літньому часі то значення відповідатиме умові");
console.log(getFormattedTime(new Date("2023-06-25T09:30:00.000Z")));
...
function compareDates(date1, date2) {
   ...
   if (
      ...
      let comparison = date1 >= date2 ? (date1 > date2 ? 1 : 0) : -1;
      return {
         date1: date1.toISOString(),
         date2: date2.toISOString(),
         comparison: comparison,
      };
   } else return "Помилка: вхіднi значення мають бути об'єктом Date";
}
...
function getDaysDifference(startDate, endDate) {
   ...
   if (
      ...
      return {
         startDate: startDate,
         endDate: endDate,
         daysDifference: diff / (1000 * 60 * 60 * 24),
      };
   } else return "Помилка: вхідне значення має бути об'єктом Date";
}
